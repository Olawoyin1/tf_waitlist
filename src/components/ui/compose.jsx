...
function caretCoords(el, pos) {
  ...
  s.wordWrap = "break-word"; s.top = "0"; s.left = "-9999px";
...
function segment(text, mentionForms, commandForms) {
  const forms = [
    ...mentionForms.map((f) => ({ f, kind: "mention" })),
    ...commandForms.map((f) => ({ f, kind: "command" })),
...
.compose-scroll { scrollbar-width: thin; scrollbar-color: rgba(161,161,170,.28) transparent; }
...
.compose-scroll::-webkit-scrollbar-thumb {
  background: rgba(161,161,170,.28); border: 4px solid transparent;
...
export function Compose({
  value, defaultValue = "", onChange, onSubmit, onCommand,
  mentions = [], commands = [],
  placeholder = "Write a message…  press @ to mention, / for commands",
  maxLength, submitLabel = "Send", autoFocus = false, className,
  ...
  const segs = React.useMemo(() => segment(text, mentionForms, commandForms), [text, mentionForms, commandForms]);
  ...
  const results = React.useMemo(() => {
    ...
    if (trigger.type === "@") return mentions.filter((m) => m.label.toLowerCase().includes(q)).slice(0, 6);
    return commands.filter((c) => c.label.toLowerCase().includes(q)).slice(0, 6);
  }, [trigger, mentions, commands]);

  const setText = React.useCallback((next) => {
  ...
  const refreshTrigger = React.useCallback(() => {
    ...
    if (t) {
      ...
      setMenu({ x: c.x - ta.scrollLeft, top: caretLineTop + c.lineHeight + 6, bottom: ta.offsetHeight - caretLineTop + 6, flip });
      setActive(0);
    }
  }, []);
  ...
  const insert = React.useCallback((choice) => {
    const ta = taRef.current;
    ...
    if (trigger.type === "/") onCommand?.(choice);
  ...
  return (
    <div className={["compose-root relative isolate w-full", className ?? ""].join(" ")}>
      ...
      <div className="relative rounded-2xl border border-zinc-200/80 bg-white shadow-[0_1px_2px_rgba(24,24,27,0.04),0_16px_40px_-24px_rgba(24,24,27,0.22)]">
        <div className="relative">
          <div ref={backdropRef} aria-hidden
            ...
            {segs.map((s, i) => {
              ...
              return (
                <span key={i} className={["box-decoration-clone rounded-[5px] py-[3px] bg-zinc-900/[0.07]", i === popIndex ? "compose-pop" : ""].join(" ")}>
                  {s.text}
                </span>
              );
            })}
          ...
          <textarea
            ...
            className={["compose-scroll relative block max-h-64 min-h-[104px] w-full resize-none bg-transparent px-4 py-3.5 text-zinc-900 caret-zinc-900 outline-none placeholder:text-zinc-400", TYPO].join(" ")}
          />
          <AnimatePresence>
            {trigger && results.length > 0 && (
              <motion.ul
                ...
                className="compose-scroll absolute z-30 max-h-[236px] w-[236px] overflow-auto rounded-xl border border-zinc-200/80 bg-white/95 p-1 shadow-[0_10px_36px_-10px_rgba(24,24,27,0.3)] backdrop-blur-xl"
                style={{ left: Math.max(8, menu.x), transformOrigin: menu.flip ? "bottom left" : "top left", ...(menu.flip ? { bottom: menu.bottom } : { top: menu.top }) }}
              >
                {results.map((r, i) => {
                        ...
                        {isMention ? (
                          <>
                            ...
                            <span className="min-w-0 flex-1 leading-tight">
                              <span className="block truncate text-[13px] font-medium text-zinc-800">{r.label}</span>
                              {r.sublabel && <span className="block truncate text-[11px] text-zinc-400">{r.sublabel}</span>}
                            </span>
                          </>
                        ) : (
                          <>
                            <span className="flex h-[26px] w-[26px] flex-none items-center justify-center rounded-[8px] bg-gradient-to-b from-zinc-50 to-zinc-100 text-zinc-700 shadow-sm ring-1 ring-inset ring-zinc-900/[0.08] [&>svg]:h-4 [&>svg]:w-4">
                              {r.icon ?? <span className="font-mono text-[13px] font-medium">/</span>}
                            </span>
                            <span className="min-w-0 flex-1 leading-tight">
                              <span className="block truncate text-[13px] font-medium text-zinc-800">{r.label}</span>
                              {r.hint && <span className="block truncate text-[11px] text-zinc-400">{r.hint}</span>}
                            </span>
                          </>
                        )}
        ...
        <div className="flex items-center justify-between gap-2 border-t border-zinc-100 px-3 py-2.5">
          ...
          <div className="flex items-center gap-3">
            ...
            <button type="button" onClick={() => text.trim() && onSubmit?.(text)}
              disabled={!text.trim() || overLimit}
              className="group inline-flex items-center gap-2 rounded-xl bg-zinc-900 px-3.5 py-2 text-[13px] font-semibold text-white shadow-sm transition-all duration-150 hover:bg-zinc-800 active:scale-[0.97] disabled:pointer-events-none disabled:opacity-40">
...
function CounterRing({ value, max }) {
  const pct = Math.min(1, value / max);
  const r = 7, circ = 2 * Math.PI * r;
  const over = value > max, near = value >= max * 0.8;
  const color = over ? "#ef4444" : near ? "#f59e0b" : "#a1a1aa";
  const remaining = max - value;
  return (
    <span className="flex items-center gap-1.5">
      {value / max >= 0.86 && <span className="tabular-nums text-[11px] font-medium" style={{ color }}>{remaining}</span>}
      <svg width="18" height="18" viewBox="0 0 18 18" className="-rotate-90">
        <circle cx="9" cy="9" r={r} fill="none" strokeWidth="2" className="stroke-zinc-200" />
        <circle cx="9" cy="9" r={r} fill="none" strokeWidth="2" stroke={color} strokeLinecap="round"
...
function Avatar({ mention }) {
  if (mention.avatar) return <img src={mention.avatar} alt="" aria-hidden className="h-6 w-6 flex-none rounded-full object-cover ring-1 ring-black/[0.06]" />;
  ...
  return <span className="flex h-6 w-6 flex-none items-center justify-center rounded-full bg-zinc-100 text-[10px] font-semibold text-zinc-500 ring-1 ring-black/[0.06]">{initials}</span>;
}

function Kbd({ children, tone }) {
  return (
    <kbd className={["inline-flex h-4 min-w-4 items-center justify-center rounded border px-1 font-sans text-[10px] leading-none",
      tone === "invert" ? "border-white/25" : "border-zinc-200 bg-zinc-50 text-zinc-500"].join(" ")}>
      {children}
    </kbd>
  );
}
